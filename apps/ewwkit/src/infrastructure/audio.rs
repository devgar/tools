...
// On each event, queries the current volume/mute state with two fast pactl calls.
// Zero overhead at idle — no polling.

#[cfg(not(feature = "alsa"))]
pub async fn get_audio() -> anyhow::Result<AudioState> {
    query_pactl().await
}
...
#[cfg(not(feature = "alsa"))]
pub fn watch_audio() -> mpsc::Receiver<AudioState> {
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            // Exclude "sink-input" events — the space before "sink #" is the differentiator.
...
//
// Build requirements: alsa-lib-devel (Fedora) / libasound2-dev (Debian/Ubuntu).
